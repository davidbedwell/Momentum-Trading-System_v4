...
def test_complete_hybrid_record_includes_gemini_work_and_analysis(tmp_path):
    ...
    (root / "reports.json").write_text(json.dumps({"reports": [{"y": 2}]}))
    ...
    assert len(work) == 4
    assert analyses == [{"analysis_id": "a1", "result": {"z": 3}}]
...
def test_blinded_packet_contains_neither_identity_nor_usage(tmp_path):
    direct = freeze_arm_manifest(subject_id="ABC", arm="DIRECT_SOL", starting_sha256="s", artifacts=[{"a": 1}], usage=[{"cost": 10}], complete=True)
    hybrid = freeze_arm_manifest(subject_id="ABC", arm="GEMINI_SOL_HYBRID", starting_sha256="s", artifacts=[{"b": 2}], usage=[{"cost": 5}], complete=True)
