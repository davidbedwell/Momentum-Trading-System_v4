...
def standard_method_catalog() -> MethodCatalog:
    return MethodCatalog([
        ...
        MethodSpec("analysis.relationship.correlation", ("NORMALIZED_DATASET",), "Measure pairwise Pearson or Spearman association without causal interpretation. Each RD-selected column is mechanically parsed as finite numeric data, including numeric strings, and only rows valid for both selected columns are retained as pairwise-complete observations; values are never imputed.", (ParameterContract("columns", True, (list, tuple), exact_length=2, meaning="two RD-selected columns; Analysis mechanically parses finite numeric values and excludes only pairwise-invalid rows"), ParameterContract("correlation_type", True, (str,), allowed_values=("pearson", "spearman"), meaning="statistic selected by RD")), 2, metadata={"numeric_observation_policy": "PAIRWISE_COMPLETE_FINITE_NO_IMPUTATION", "scientific_selection": "AI_RESEARCH_DIRECTOR_ONLY"}),
