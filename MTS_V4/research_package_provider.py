        ...
        *,
        ...
        user["instructions"].extend(
            [
                ...
                "Keep three judgments distinct: trading-hypothesis candidacy, scientific validation, and candidacy for trading promotion. A scientific observation is not automatically a candidate trading hypothesis, and scientific validation is not automatically trading promotion.",
                "Create action=CREATE_TENTATIVE for a candidate trading hypothesis only when exploratory evidence supports a specific falsifiable T-to-T+1 relationship and a proposed human-executable policy has positive estimated net expectancy after estimated costs. The frozen proposition and success_definition must identify entry, direction, risk/stop or invalidation, favorable exit, maximum horizon, path order, gap/fill treatment, costs, and applicability sufficiently to prevent crediting movement after the policy would have exited. Include a scientifically appropriate positive minimum_required_trials and supporting source_result_ids.",
                "Historical look-ahead is allowed for candidate discovery but must not turn an eventual destination into an achievable trade: never credit favorable movement after a stop, invalidation, expiry, or other policy exit. A predictive relationship without positive exploratory policy expectancy may remain a scientific observation or supporting finding, but not a candidate trading hypothesis.",
                ...
                "The former fixed cumulative success_rate >= 0.60 rule is superseded and must not be treated as the exclusive scientific-validation or trading-promotion criterion. Greater-than-60-percent success at at-least-1.5-ATR remains a strong reference payoff example only. You retain scientific authority to validate, qualify, reject, or leave unresolved the frozen predictive relationship using untouched evidence, including magnitude, uncertainty, dependence, selection effects, stability, adverse path, and reproducibility.",
                "Trading promotion is a separate later judgment. A scientifically validated hypothesis is eligible for prospective paper testing only when frozen path-executable evidence shows conservative net expectancy > 0, point-estimate net expectancy >= 0.15R, and point-estimate gross expectancy >= 2C, where R is the frozen adverse-risk unit and C is upper-bound all-in cost. This is a rejection floor, not a research target, attractiveness judgment, or live-trading authority. The present binary trial ledger does not yet represent that full promotion assessment; do not claim that its status alone satisfies the amended gate.",
                "For future screening, distinguish eligibility from ranking and capital selection. Preserve every eligible candidate, including candidates not selected for capital, so prospective hypothesis evidence is not biased by portfolio constraints. Ranking may compare conservative expectancy, capital efficiency, holding time, adverse path, uncertainty, liquidity, regime, cost sensitivity, capacity, and correlation, but may not silently alter the frozen hypothesis.",
