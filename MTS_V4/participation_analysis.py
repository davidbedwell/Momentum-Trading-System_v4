...
def participation_context(evidence_payloads: Mapping[str, object], parameters: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    for index, row in enumerate(ordered):
        ...
        panel.append({
            ...
            "candidate_event_count": len(matches),
            "candidate_event_types": sorted({str(item.get("event_type")) for item in matches if item.get("event_type")}),
            "candidate_headlines": [str(item.get("headline")) for item in matches if item.get("headline")][:20],
        })
    ...
    return {
        ...
        "latest_session": dict(panel[-1]) if panel else {},
        "historical_float_limitation": "Current float is not applied to historical dates; dated historical float is required for historical float interaction testing.",
        "event_boundary": "Event proximity is measured without assigning market significance or causation.",
        "derived_dataset_catalog": {"participation_event_panel": {"row_count": len(panel), "schema": list(panel[0]) if panel else [], "output_path": ["derived_datasets", "participation_event_panel"], "temporary": True}},
...
def method_spec() -> MethodSpec:
    return MethodSpec(
        METHOD_ID,
        ("NORMALIZED_DATASET",),
        "Expose continuous float, relative-volume, and event-proximity measurements for Research Director evaluation.",
        (
            ParameterContract("adv_window", True, (int,), minimum_value=2, maximum_value=252),
            ParameterContract("event_window_days", True, (int,), minimum_value=0, maximum_value=10),
        ),
        2,
        True,
        True,
        False,
        {"scientific_selection": "AI_RESEARCH_DIRECTOR_ONLY", "deterministic_ranking": False, "reusable_derived_dataset": True, "derived_dataset_name": "participation_event_panel", "historical_float_requires_dated_source": True},
    )
