...
Historical earnings-event evidence is acquired through Intake from the EODHD Corporate Events Calendar. The provider's `BeforeMarket` and `AfterMarket` classifications are preserved as categorical availability evidence; deterministic code may align `BeforeMarket` to that observed session's close and `AfterMarket` to the next observed session's close, but it may not invent an exact release timestamp. Records with unknown timing are excluded from predictor publication and counted in a frozen coverage audit. Associated historical consensus estimates are labeled as provider-associated estimates rather than independently reconstructed estimate vintages. Acquisition must be resumable, must not persist API credentials, and must pass declared universe-coverage and known-timing thresholds before an earnings feature stream is published.
