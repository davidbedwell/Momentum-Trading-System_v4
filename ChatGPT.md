...
Historical earnings-event evidence is acquired through Intake from the EODHD Corporate Events Calendar. The provider's `BeforeMarket` and `AfterMarket` classifications are preserved as categorical availability evidence; deterministic code may align `BeforeMarket` to that observed session's close and `AfterMarket` to the next observed session's close, but it may not invent an exact release timestamp. A record with unknown timing is conservatively delayed to the next observed session close after its report date and marked `earnings_timing_known=0`; known timing is marked `1`, so scientific work can stratify or exclude lower-precision records. Associated historical consensus estimates are labeled as provider-associated estimates rather than independently reconstructed estimate vintages. Acquisition must be resumable, must not persist API credentials, and must pass declared universe-coverage and safe-alignment checks before an earnings feature stream is published.
...
## 11-Ticker Sol Revisit Authorization and Learning Audit — 2026-09-20

The human authorized the required AAPL→AMD→AMZN→BA→GOOGL→JPM→META→MSFT→NVDA→TSLA→XOM revisit sequence with a **$7.00 hard Sol ceiling per ticker ($77 maximum)**. The expected cost-efficiency range remains approximately $5–$6/ticker and is not a scientific target or truncation instruction.

The sequence must produce a terminal learning audit covering governance compliance, Sol's use of prior/established predictive structures, exact opportunity frequency per observed month when evidence supports it, Sol-authored genuinely novel strategy claims, and evidence of cross-subject market-structure generalization or contradiction across the ordered eleven subjects. Deterministic code aggregates these annotations and objective telemetry but does not decide scientific novelty/generalization. Full authority and methodology are frozen in `Documentation/2026-09-20_11_TICKER_SOL_LEARNING_AUDIT.md`.


## Required Known Predictive Theory Coverage — 2026-09-20

The human explicitly requires the eleven-ticker Sol revisit to test established predictive theory families rather than merely expose them as optional background. Every catalog `theory_id` must receive a terminal Sol-authored disposition before subject closure: `TESTED_SUPPORTED`, `TESTED_UNSUPPORTED`, `TESTED_MIXED`, or `OBJECTIVELY_NOT_TESTABLE` when the required evidence truly is unavailable. Required coverage gives the theory no privileged evidentiary weight and does not prescribe method, threshold, horizon, direction, interpretation, significance, or trading candidacy. Sol retains those scientific judgments and remains unrestricted in novel discovery. Authoritative detail: `Documentation/2026-09-20_KNOWN_PREDICTIVE_THEORY_COVERAGE.md`.
