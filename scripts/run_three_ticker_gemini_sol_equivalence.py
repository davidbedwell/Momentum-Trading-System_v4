...
def main() -> int:
    ...
    p.add_argument("--repo-root", default="/home/ubuntu/Momentum-Trading-System_v4")
    ...
    # Identity map is private until each blinded scientific comparison is completed.
    salt = os.urandom(32).hex()
    identity_path = exp / "BLIND_IDENTITY_SECRET.json"
    if identity_path.exists():
        raise EquivalenceProtocolError("experiment already started; refusing accidental rerun")
    ...
    for ticker in tickers:
        ...
        start = prepare_identical_start(root=repo, ticker=ticker, experiment_root=exp)
        direct = run_direct_sol_arm(root=repo, ticker=ticker, experiment_root=exp, start=start, sol_spend_usd=args.direct_sol_cap)
        gemini_root, gemini_usage, _ = run_gemini_rd_arm(root=repo, ticker=ticker, experiment_root=exp, start=start, model=args.gemini_model, max_calls=args.gemini_max_calls, max_spend_usd=args.gemini_cap)
        ...
        appeal = run_bounded_sol_appeal(start=start, gemini_root=gemini_root, hybrid_root=hybrid_root, sol_provider=appeal_provider, max_sol_calls=1, max_sol_spend_usd=args.appeal_sol_cap)
        # The generic appellate transport currently lacks authoritative usage telemetry.
        # Fail closed rather than fabricate cost; its explicit cap is charged for qualification.
        ...
        # A separate blinded adjudicator is required; do not let either research arm grade itself.
        ...
        # Reveal identity only after the blind verdict is frozen.
        ...
        if missed or false_hybrid or not comparison["materially_equivalent"]:
            print("STOP: scientific equivalence failed; remaining paid subjects not run.", flush=True)
